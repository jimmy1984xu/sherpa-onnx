...
def _is_ascii_word(token: str) -> bool:
    return _ASCII_WORD_RE.fullmatch(token) is not None


def _collapse_space_equivalent_alignment(
    ali: List[Tuple[str, str]], err_token: str
) -> List[Tuple[str, str]]:
    """Collapse exact spacing-only differences in an existing alignment.

    This runs after the normal edit-distance alignment, so insertions and
    deletions before a spacing variant cannot shift the matching positions.
    """
    collapsed: List[Tuple[str, str]] = []
    index = 0

    while index < len(ali):
        ref_word, hyp_word = ali[index]
        if ref_word == hyp_word:
            collapsed.append((ref_word, hyp_word))
            index += 1
            continue
        ...
        while end < len(ali):
            current_ref, current_hyp = ali[end]
            if current_ref == current_hyp:
                break

            if current_ref != err_token:
                ref_words.append(current_ref)
            if current_hyp != err_token:
                hyp_words.append(current_hyp)

            if len(ref_words) > 1 and len(hyp_words) > 1:
                break

            if len(ref_words) == 1 and len(hyp_words) >= 2:
                if (
                    all(_is_ascii_word(word) for word in ref_words + hyp_words)
                    and ref_words[0] == "".join(hyp_words)
                ):
                    match_end = end + 1
                    canonical = ref_words[0]
                    break

            if len(hyp_words) == 1 and len(ref_words) >= 2:
                if (
                    all(_is_ascii_word(word) for word in ref_words + hyp_words)
                    and hyp_words[0] == "".join(ref_words)
                ):
                    match_end = end + 1
                    canonical = hyp_words[0]
                    break

            end += 1

        if match_end is not None and canonical is not None:
            collapsed.append((canonical, canonical))
            index = match_end
        else:
            collapsed.append((ref_word, hyp_word))
            index += 1
...
def write_error_stats(
    ...
    for cut_id, ref, hyp in results:
        ...
        if not compute_CER:
            ali = _collapse_space_equivalent_alignment(ali, ERR)
        err = 0
        a_del = 0
        a_ins = 0
