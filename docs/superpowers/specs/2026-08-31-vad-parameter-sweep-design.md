...
以四个音频合计的加权 WER 作为主指标：

`weighted_wer = 100 * sum(err_words) / sum(ref_words)`

使用错误词数和参考词数重新计算，避免对不同长度音频做简单平均造成偏差。

### 稳定性检查

每个配置同时保留单音频 WER。若某配置虽然加权 WER 较低，但导致任一音频出现明显异常退化，应在报告中标注，不直接作为最终推荐。若两个配置加权 WER 接近，优先选择单音频波动更小、且参数更简单的配置。
...
每个阶段目录下按音频名称分目录；每次单次运行保留脚本生成的 `segments/`、`asr.txt`、`merged.txt` 和 `wer_detail.txt`。汇总文件至少包含：阶段、音频、`min_silence_duration`、`pre_speech_pad_duration`、`merge_gap_duration`、WER、参考词数、错误词数、删除数、插入数和明细文件路径。

`vad_parameter_report.md` 需要包含：

1. 分支、commit、模型和数据集信息；
2. 四个阶段的候选参数及每个候选的总体/单音频结果；
3. 每个阶段选中的参数及选择依据；
4. 最终推荐组合和加权 WER；
5. 异常音频、失败运行或中断信息。
