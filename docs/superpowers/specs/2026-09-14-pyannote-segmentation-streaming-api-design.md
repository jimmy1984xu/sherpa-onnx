...
```text
[start, end, speaker_count]
```

其中 `speaker_count` 是 `0`（无活动）、`1`（单人）或 `2`（重叠说话）。相邻而
`speaker_count == 1` 的 span 若没有合并，表示它们之间已经确认存在单人换人候选边界。
...
- 两个相邻结果不得时间重叠；
- `speaker_count ∈ {0, 1, 2}`；
- 除 `InputFinished()` 的末尾补零窗口外，已输出 span 不会被后续输入修改或撤回。

### 4.2 相邻同人数 span 的强约定

正常情况下，相邻且 `speaker_count` 相同的 span 必须合并。唯一例外是：

```text
previous.speaker_count == 1
current.speaker_count  == 1
previous.end == current.start
```

且中间边界已经被多窗口 change-boundary fusion 确认。这表示：

```text
单人 A -> 单人 B 的候选换人边界
```

这里的 A/B 只表示各窗口中的 local speaker track，不代表全局 speaker ID。上层可将
这类相邻 `1 -> 1` 边界作为 ASR 断句候选；是否最终切分由上层 VAD、ASR token 边界和
自身的全局 embedding/clustering 决定。

`0 -> 1`、`1 -> 0`、`1 -> 2`、`2 -> 1` 等相邻 count 不同的边界，表示活动人数或
重叠状态变化。
...
不尝试对齐不同窗口的 local bit。每个窗口独立判断边界 `t` 前后是否满足：

```text
左侧：持续至少 min_change_duration，人数恒为 1，且唯一活跃 local bit 恒为 A；
右侧：持续至少 min_change_duration，人数恒为 1，且唯一活跃 local bit 恒为 B；
A != B。
```
...
```text
min_change_duration = 0.30 s
```

该参数同时约束边界左右两侧；它抑制逐帧 track 抖动，不等价于 VAD 静音阈值。
...
change_coverage[frame]  # 覆盖并能够观察该位置左右稳定区的窗口数
```
...
并只保留局部最大值；两个确认事件距离小于 `min_change_duration` 时，仅保留支持率更高
的一个。这能把不同窗口的 `4.98 s`、`5.00 s`、`5.03 s` 聚合为一个边界。
...
按边界拆分出 `[start, end, speaker_count]`，并执行以下合并规则：

- count 不同：保留边界；
- count 相同且为 `0` 或 `2`：合并；
- count 相同且为 `1`：仅在确认的单人换人边界处保留切分，否则合并。

这使上层可以只根据 span 序列区分 silence、single speaker、overlap 和单人换人候选，
无需消费 raw window 或 local mask。
...
每个对象拥有独立的模型会话、音频缓存、帧融合器、pending span 和结果队列；不存在
...
已经最终确定且已入队的时间区域会被丢弃，因此连续长音频不会按总时长线性增长内存。
...
```text
输入达到 10 s：可最终确定 [0 s, 1 s)；
输入达到 11 s：可最终确定 [1 s, 2 s)；
输入达到 15 s：可最终确定 [5 s, 6 s)。
```

因此稳定输出的算法延迟约为一个模型窗口，即约 10 秒，而不是额外等待 9 个窗口。

完整 span 必须在其右边界最终确定后才会入结果队列；如果一段 `speaker_count == 1`
持续很长而没有后续边界，该 span 会保持为对象内部 pending span，直到出现已确认边界
或调用 `InputFinished()`。不会为了“尽快出结果”而把相同 count 的长 span 人为切块，
从而避免上层把人工切块误解为换人边界。
...
struct SpeakerSegmentationConfig {
  ...
  float min_change_duration = 0.30f;
...
config.min_change_duration = 0.30
...
for samples in pcm_blocks_32ms:
    ...
    while not segmenter.empty():
        ...
        # span.start, span.end, span.speaker_count
        segmenter.pop()
...
Python `SpeakerSegmentationSpan` 提供只读属性：`start`、`end`、`speaker_count`。
...
4. 只有一个窗口报告的短抖动，不满足稳定时长或投票阈值时不切分；
5. overlap（`count == 2`）正确输出；
6. 相邻同 count span 仅允许确认的 `1 -> 1` 换人边界；
7. 32 ms 分块输入与一次性输入得到相同最终 spans；
8. `InputFinished()` 的尾窗补零、实际音频长度裁剪和 `Reset()` 复用；
9. 多对象状态隔离。
