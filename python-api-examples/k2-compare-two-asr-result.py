...
"""
比较两个ASR结果文件并将结果合并到Excel文件。

该脚本读取两个ASR结果文件（由 k2-asr-short.py 生成），
将它们合并到一个Excel文件中，方便对比分析。

重要要求：
- 两个输入文件的行数必须完全一致，否则会抛出异常
- 两个文件的segment_id值和顺序必须完全一致
...
- segment_id: 音频片段ID
- text1: 第一个ASR结果的文本
- confidence1: 第一个ASR结果的置信度（如果有）
- text2: 第二个ASR结果的文本
- confidence2: 第二个ASR结果的置信度（如果有）
- confidence_diff: 置信度差异（confidence1 - confidence2）
- diff_level: 差异度等级（高/中/低/错）

使用示例：

python3 ./python-api-examples/k2-compare-two-asr-result.py \
