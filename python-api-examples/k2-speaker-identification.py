...
"""
Extract speaker embeddings from audio files listed in wav.scp.

This script reads a wav.scp file (format: segment_id path/to/audio.wav),
extracts speaker embeddings for each audio file using a speaker embedding model,
and outputs the results to a text file with two columns: id and serialized embedding.

Usage example:

python3 ./python-api-examples/k2-speaker-identification.py \
...
- If audio duration exceeds --max-audio-duration, only the first N seconds will be used for embedding extraction to reduce computational cost
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        ...
        description="Extract speaker embeddings from audio files in wav.scp"
    )
