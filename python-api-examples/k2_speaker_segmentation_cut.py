...
"""
Cut long audio into speaker segments using only speaker segmentation model (no embedding/clustering).

This script uses only the segmentation model to detect speaker activities and extract segments.
Note: Without clustering, speaker IDs are temporary indices from the segmentation model,
not unique speaker identities.

Usage example:

python3 ./python-api-examples/k2_speaker_segmentation_cut.py \
...
Notes:
- Only segmentation model is required (no embedding model needed).
- Segments are saved as WAV files (16 kHz) under the specified output directory.
- wav.scp will contain lines in the format: <segment_id> <absolute_path>.
- Segment filenames: <audio_name>_<start_ms>_<duration_ms>.wav
- Segment ID to Speaker ID mapping is printed in the log.
- Speaker indices are temporary (from segmentation model), not unique speaker identities.

Parameters:
- --min-duration-on: Minimum segment duration in seconds (default: 0.2)
  * Segments shorter than this value will be filtered out (discarded).
  * Use a smaller value (e.g., 0.1) to keep more short segments.
  * Use a larger value (e.g., 0.5) to filter out very short segments (noise, false positives).
  
- --min-duration-off: Minimum gap between segments to merge in seconds (default: 0.5)
  * If two segments from the same speaker are separated by a gap <= this value, they will be merged into one segment.
  * Use a smaller value (e.g., 0.2) to merge more aggressively (fewer segments).
  * Use a larger value (e.g., 1.0) to merge less aggressively (more segments, keep gaps).
  * Set to 0 to disable merging (keep all gaps).
"""
